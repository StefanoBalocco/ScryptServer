...
const ScryptServer = serverModule.ScryptServer as new ( config: unknown ) => {
	request: ( path: string, options?: RequestInit ) => Promise<Response>;
	hash: ( data: string, params: unknown ) => Promise<{ error?: string; result?: string }>;
	compare: ( data: string, hash: string ) => Promise<{ error?: string; result?: boolean }>;
	Start: () => Promise<void>;
	Stop: () => Promise<void>;
};

// Unit tests for ScryptServer class
...
// HTTP handler tests (via app.request)
...
test.serial( 'HTTP /compare: verifies correct password', async( t ) => {
	...
	const hashBody = await hashResponse.json() as { result?: string };
	...
	const compareBody = await compareResponse.json() as { result?: boolean };
